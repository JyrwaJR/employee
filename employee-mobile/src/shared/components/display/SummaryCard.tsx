...
export const SummaryCard = ({ label, amount, details, className, variant = 'blue' }: SummaryCardProps) => {
  const bgClass = variant === 'green' ? 'bg-green-600 dark:bg-green-700' : 
                  variant === 'red' ? 'bg-red-600 dark:bg-red-700' : 
                  'bg-blue-600 dark:bg-blue-700';
  
  const textClass = variant === 'green' ? 'text-green-100' : 
                    variant === 'red' ? 'text-red-100' : 
                    'text-blue-100';

  const borderClass = variant === 'green' ? 'bg-green-500/50' : 
                      variant === 'red' ? 'bg-red-500/50' : 
                      'bg-blue-500/50';
